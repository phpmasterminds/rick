...
export async function GET(request: NextRequest) {
  try {
    ...
    console.log('[discount-types] page_id:', page_id);
    ...
    const response = await axios.get("/business/pos-inventory/membership-levels", {
      params: {
        page_id: page_id
      }
    });
    ...
    return NextResponse.json(
      { 
        status: 'failed',
        message: error.response?.data?.message || "Failed to fetch membership levels" 
      },
