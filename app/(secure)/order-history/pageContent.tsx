...
interface Order {
  ...
  sales_person?: string; // Sales person ID for dropdown
...
const ActionMenu: React.FC<ActionMenuProps> = ({ order, business, onPaymentClick }) => {
  ...
  const handleEditOrder = () => {
    router.push(`/order-history/${order.order_id}/edit`);
    setIsOpen(false);
  };
...
export default function OrderPageContent({ business, typeid }: { business: string, typeid: string }) {
  ...
  return (
    <div className="flex-1 p-4 md:p-6 overflow-auto bg-gradient-to-b from-gray-50 to-gray-100 dark:from-gray-950 dark:to-gray-900">
      {/* Header */}
      <div className="mb-6">
        <h1 className="text-2xl font-bold text-gray-900 dark:text-gray-100 capitalize">Orders history</h1>
                    ...
                    <td className="px-6 py-4">
                      <div>
                        <p className="font-bold text-blue-600 dark:text-blue-400">
							<Link href={`/order-history/${order.order_id}`}>
								{order.order_id}
							</Link>
						</p>
