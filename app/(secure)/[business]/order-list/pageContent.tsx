...
const ActionMenu: React.FC<ActionMenuProps> = ({ order, business, onPaymentClick }) => {
  ...
  const handleViewOrder = () => {
    router.push(`/${business}/order-list/${order.order_id}`);
    setIsOpen(false);
  };

  const handleEditOrder = () => {
    router.push(`/${business}/order-list/${order.order_id}/edit`);
    setIsOpen(false);
  };
...
export default function OrderPageContent({ business }: { business: string }) {
  ...
  const filteredOrders = useMemo(() => {
    // Currently just returning orders as-is since API handles pagination
    // Uncomment below if you want client-side filtering
    ...
    filtered.sort((a, b) => {
      ...
      if (typeof aVal === 'string' && typeof bVal === 'string') {
        return sortConfig.ascending ? aVal.localeCompare(bVal) : bVal.localeCompare(aVal);
      }
    ...
    */
    ...
    // For server-side pagination, return orders as-is
    return orders;
      ...
      <div className="bg-white dark:bg-gray-900 rounded-lg p-6 border border-gray-200 dark:border-gray-800 mb-6">
        ...
        <div className="flex gap-4 mb-6">
          ...
          <div>
            ...
            <button className="px-6 py-3 text-white rounded-lg font-semibold transition-all duration-300 hover:scale-105 accent-bg accent-hover">
              <Search size={20} />
            </button>
          </div>
        </div>
        ...
        <div className="flex flex-wrap gap-3 items-end">
          <div>
            <label className="block text-sm font-semibold text-gray-700 dark:text-gray-300 mb-2">Filter by</label>
            <button className="px-4 py-2 border border-gray-300 dark:border-gray-700 rounded-lg text-sm font-medium text-gray-700 dark:text-gray-300 hover:bg-gray-50 dark:hover:bg-gray-800 flex items-center gap-2">
              <Filter size={16} />
              Filters <span className="ml-1 bg-blue-600 text-white rounded-full px-2 text-xs">1</span>
            </button>
          </div>
          ...
          <div>
            <select
              ...
              className="px-4 py-2 border border-gray-300 dark:border-gray-700 rounded-lg text-sm font-medium text-gray-700 dark:text-gray-300 dark:bg-gray-800 hover:bg-gray-50 dark:hover:bg-gray-700"
            >
          ...
          <div>
            <button className="px-4 py-2 border border-gray-300 dark:border-gray-700 rounded-lg text-sm font-medium text-gray-700 dark:text-gray-300 hover:bg-gray-50 dark:hover:bg-gray-800">
              Customer
          ...
          <button className="ml-auto text-blue-600 hover:text-blue-700 font-semibold text-sm">Reset all</button>
        </div>
      </div>

      {/* Table Section */}
      <div className="bg-white dark:bg-gray-900 rounded-lg border border-gray-200 dark:border-gray-800 overflow-hidden">
        {/* Header with Sort and Download */}
        <div className="px-6 py-4 border-b border-gray-200 dark:border-gray-800 flex justify-between items-center bg-gray-50 dark:bg-gray-800 rounded-t-lg">
          <div className="flex gap-6">
            <button className="flex items-center gap-2 text-blue-600 hover:text-blue-700 dark:text-blue-400 dark:hover:text-blue-300 font-semibold transition">
			  <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" className="w-5 h-5">
				<path d="M7 3h10M3 9h18M5 15h14M6 21h12" strokeWidth={2} strokeLinecap="round" />
			  </svg>
			  Sort
			</button>
            <button className="flex items-center gap-2 text-blue-600 hover:text-blue-700 dark:text-blue-400 dark:hover:text-blue-300 font-semibold transition">
                    ...
                    <td className="px-6 py-4">
                      <div>
                        <p className="font-bold text-blue-600 dark:text-blue-400">
							<Link href={`/${business}/order-list/${order.order_id}`}>
								{order.order_id}
							</Link>
						</p>
