...
const ActionMenu: React.FC<ActionMenuProps> = ({ order, business, onPaymentClick }) => {
  ...
  const handleViewOrder = () => {
    router.push(`/open-orders/${order.order_id}`);
    setIsOpen(false);
  };

  const handleEditOrder = () => {
    router.push(`/open-orders/${order.order_id}/edit`);
    setIsOpen(false);
  };
...
const StatusDisplay: React.FC<{ order: Order }> = ({ order }) => {
	  ...
	  const fetchOrders = async () => {
		try {
		  ...
		  const response = await axios.get(
			`/api/business/save-whole-sale-order/?business=${business}&page=${currentPage}&wholesale=${wholesale}&open_orders=1`
		  );
  ...
  const filteredOrders = useMemo(() => {
    ...
    let filtered = orders.filter((order) => {
      const searchMatch =
        ...
        (order.seller_information?.title ?? '').toLowerCase().includes(searchTerm.toLowerCase()) ||
                    ...
                    <td className="px-6 py-4">
                      <div>
                        <p className="font-bold text-blue-600 dark:text-blue-400">
							<Link href={`/open-orders/${order.order_id}`}>
								{order.order_id}
							</Link>
						</p>
                      </div>
